...
void UpdateDialog::accept()
{
    ...
    for (const QJsonValue& value : jsonArray)
    {
        ...
        #ifdef _WIN32
        if (filenameBlob.contains(QStringLiteral("SG.Windows.Update")) || 
            filenameBlob.contains(QStringLiteral("SG.Windows")) || 
            filenameBlob.contains(QStringLiteral("SG.win64")))
        {
        ...
        #endif
        #ifdef __APPLE__
        if (filenameBlob.contains(QStringLiteral("SG.macOS.Update")) || 
            filenameBlob.contains(QStringLiteral("SG.macOS")))
        {
            filenameToDownload = filenameBlob;
            urlToDownload = downloadUrl;
            break;
